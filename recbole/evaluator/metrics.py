...
class MRR(TopkMetric):
    ...
    def calculate_metric(self, dataobject, unbiased=False):
        if unbiased:
            ...
            pscore = pscore.numpy()
            result = self.metric_info_unbiased(pos_index, pscore)
    ...
    def metric_info_unbiased(self, pos_index, pscore):
        idxs = pos_index.argmax(axis=1)
        result = np.zeros_like(pos_index, dtype=np.float)
        for row, idx in enumerate(idxs):
            if pos_index[row, idx] > 0:
...
class Recall(TopkMetric):
    ...
    def calculate_metric(self, dataobject, unbiased=False):
        if unbiased:
            ...
            pscore = dataobject.get('data.pscore')
            pscore = pscore.numpy()
            result = self.metric_info_unbiased(pos_index, pos_len, pscore)
    ...
    def metric_info_unbiased(self, pos_index, pos_len, pscore):
