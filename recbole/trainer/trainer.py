...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.unbiased_eval = config["unbiased_eval"]
    ...
    def _unbiased_valid_epoch(self, valid_data, show_progress=False, pscore=None):
        ...
        Returns:
            ...
            dict: valid result
        """
        valid_result = self.unbiased_evaluate(valid_data, load_best_model=False, show_progress=show_progress, pscore=pscore)
        valid_score = calculate_valid_score(valid_result, self.valid_metric)
        return valid_score, valid_result
    ...
    def fit(
        ...
        pscore = train_data.dataset.pscore()
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if self.unbiased_eval:
                    valid_score, valid_result = self._unbiased_valid_epoch(
                        valid_data, show_progress=show_progress, pscore=pscore
                    )
                else:
    ...
    def unbiased_evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False, pscore=None):
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            self.eval_collector.eval_batch_collect_unbiased(scores, interaction, positive_u, positive_i, pscore) # top k에 몇개 아이템 hit하엿는지
        self.eval_collector.model_collect(self.model)
        struct = self.eval_collector.get_data_struct_unbiased() # 이미 struct 만들 때 각 user 별로 top k에 hit를 몇 개의 item이 하였는지를 struct에서 갖고 있음
