...
class Dataset(torch.utils.data.Dataset):
    ...
    def split_matrix_by_interactions(self, num_windows):
        # sequence x max_length matrix (timestamp)
        seq_time_matrix = np.concatenate((self.inter_feat.timestamp_list, \
                                          self.inter_feat.timestamp.reshape(-1, 1)), axis=1)
        
        # sequence x max_length matrix (item_id)
        seq_item_matrix = np.concatenate((self.inter_feat.item_id_list, \
                                          self.inter_feat.item_id.reshape(-1, 1)), axis=1)
    
        # 0이 아닌 요소의 인덱스와 해당 값들을 찾습니다.
        nonzero_indices = np.nonzero(seq_time_matrix)
        nonzero_values = seq_time_matrix[nonzero_indices]
        
        # timestamp 값들을 정렬합니다.
        sorted_timestamps = np.sort(nonzero_values)
        
        # 분할할 각 그룹의 대략적인 크기를 계산합니다.
        group_size = len(sorted_timestamps) // num_windows
        
        # 각 분할에 대한 시작점을 결정합니다.
        split_indices = [i * group_size for i in range(num_windows)] + [len(sorted_timestamps)]
        
        split_matrices = []
        for i in range(len(split_indices) - 1):
            # 각 분할에 대한 timestamp 범위를 결정합니다.
            start, end = split_indices[i], split_indices[i+1]
            split_range = (sorted_timestamps[start], sorted_timestamps[end-1])
            
            # 원본 행렬에서 해당 범위에 해당하는 interaction만을 포함하는 새 행렬을 생성합니다.
            split_matrix = np.where((seq_time_matrix >= split_range[0]) & (seq_time_matrix <= split_range[1]), \
                                    seq_time_matrix, 0)
            split_matrix = np.where(split_matrix > 0, 1, 0)

            split_matrices.append(split_matrix)
        
        # 각 window에 대한 item별 인기도를 계산합니다.           
        pop_by_window = np.zeros((num_windows, self.item_num))
        for i in range(num_windows):
            split_seq_item_matrix = seq_item_matrix * split_matrices[i]
            filtered_matrix = split_seq_item_matrix[split_seq_item_matrix != 0]
            unique_items, counts = np.unique(filtered_matrix, return_counts=True)
            pop_by_window[i][unique_items] = counts

        return pop_by_window
